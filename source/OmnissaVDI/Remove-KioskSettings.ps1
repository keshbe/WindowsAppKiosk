[CmdletBinding()]
param (
    [string]$EventLog = 'Omnissa-VDI-Kiosk',
...
$ScheduledTasks = Get-ScheduledTask | Where-Object { $_.TaskName -like '(Omnissa VDI)*' }
If ($ScheduledTasks) {
...
$DirsShortcuts = "$env:ProgramData\Microsoft\Windows\Start Menu\Programs", "$env:ProgramData\Microsoft\Windows\Start Menu\Programs\Startup", "$env:SystemDrive\Users\Public\Desktop", "$env:SystemDrive\Users\Default\AppData\Roaming\Microsoft\Windows\Start Menu\Programs\Startup"
$linksKiosk = "Omnissa Horizon Client.lnk", "Horizon Fullscreen.lnk", "ISL Light Client.lnk"
ForEach ($DirShortcut in $DirsShortcuts) {
    ForEach ($linkKiosk in $linksKiosk) {
        $pathLink = Join-Path $DirShortcut -ChildPath $linkKiosk
        If (Test-Path -Path $pathLink) {
            Write-Log -EventLog $EventLog -EventSource $EventSource -EventId 20 -EntryType Information -Message "Removing kiosk shortcut: '$pathLink'."
            Remove-Item -Path $pathLink -Force
        }
    }
}
