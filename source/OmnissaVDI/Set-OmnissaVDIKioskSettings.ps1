...
#region Deploy Fullscreen Launcher + Start Menu Shortcut
# The Horizon Client ignores the "start maximized" hint, so a small launcher maximizes its window
# via the Win32 API after it appears. Deploy the launcher to KioskSettings and bake in the values.
$LauncherDst     = Join-Path $DirKiosk 'Launch-HorizonFullscreen.ps1'
$LauncherContent = Get-Content -Path (Join-Path $DirCustomLaunch 'Launch-HorizonFullscreen.ps1') -Raw
$LauncherContent = $LauncherContent.Replace('{{HORIZON_CLIENT_PATH}}',    $HorizonClientPath)
$LauncherContent = $LauncherContent.Replace('{{HORIZON_SERVER_URL}}',     $HorizonServerUrl)
$LauncherContent = $LauncherContent.Replace('{{HORIZON_DESKTOP_LAYOUT}}', $HorizonDesktopLayout)
$LauncherContent = $LauncherContent.Replace('{{HORIZON_DESKTOP_NAME}}',   $HorizonDesktopName)
$LauncherContent | Out-File -FilePath $LauncherDst -Encoding utf8 -Force

# The Start Menu shortcut launches the (hidden) launcher, which starts and maximizes the client.
$PowerShellExe = "$env:SystemRoot\System32\WindowsPowerShell\v1.0\powershell.exe"
$ObjShell = New-Object -ComObject WScript.Shell
$Shortcut = $ObjShell.CreateShortcut($LnkPath)
$Shortcut.TargetPath       = $PowerShellExe
$Shortcut.Arguments        = "-ExecutionPolicy Bypass -WindowStyle Hidden -NonInteractive -File `"$LauncherDst`""
...
# Auto-launch at logon: drop the shortcut into the Default user's Startup folder.
...
Write-Log -EventLog $EventLog -EventSource $EventSource -EntryType Information -EventId 45 -Message "Deployed fullscreen launcher to '$LauncherDst' and created the auto-launch shortcut."
#endregion
...
# Taskbar auto-hide for the kiosk user: seed StuckRects3 in the Default profile so KioskUser0
# inherits an auto-hiding taskbar (it slides away and is revealed - together with the Wi-Fi
# flyout - by moving the mouse to the bottom edge). Byte 8 = 0x03 enables auto-hide; byte 12 = 0x03
# keeps it on the bottom edge. This 48-byte layout matches Windows 11 (build 26200); explorer
# recomputes the cached taskbar rectangle for the actual display, so it need not match here.
If (Test-Path -Path 'HKLM:\Default') {
    $TaskbarAutoHide = [byte[]](0x30,0x00,0x00,0x00, 0xFE,0xFF,0xFF,0xFF, 0x03,0x00,0x00,0x00, 0x03,0x00,0x00,0x00,
                                0x90,0x00,0x00,0x00, 0x54,0x00,0x00,0x00, 0x00,0x00,0x00,0x00, 0xB4,0x06,0x00,0x00,
                                0x40,0x0B,0x00,0x00, 0x08,0x07,0x00,0x00, 0xA8,0x00,0x00,0x00, 0x01,0x00,0x00,0x00)
    Set-RegistryValue -Path 'HKLM:\Default\Software\Microsoft\Windows\CurrentVersion\Explorer\StuckRects3' -Name 'Settings' -PropertyType 'Binary' -Value $TaskbarAutoHide
    Write-Log -EventLog $EventLog -EventSource $EventSource -EntryType Information -EventId 105 -Message "Enabled taskbar auto-hide for the kiosk user (StuckRects3 seeded in the Default profile)."
}
