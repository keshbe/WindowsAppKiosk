...
If (Test-Path -Path $DirKiosk) {
    ...
    If ($null -ne $FileRegValuesRestore) {
        ...
        If ($HiveLoaded) {
            # Remove the taskbar auto-hide that was seeded into the Default profile at install time.
            Remove-ItemProperty -Path 'HKLM:\Default\Software\Microsoft\Windows\CurrentVersion\Explorer\StuckRects3' -Name 'Settings' -Force -ErrorAction SilentlyContinue
...
$linkHorizon = "Omnissa Horizon Client.lnk"
ForEach ($DirShortcut in $DirsShortcuts) {
    $pathLinkHorizon = Join-Path $DirShortcut -ChildPath $linkHorizon
    If (Test-Path -Path $pathLinkHorizon) {
        Write-Log -EventLog $EventLog -EventSource $EventSource -EventId 20 -EntryType Information -Message "Removing Horizon Client Shortcut: '$pathLinkHorizon'."
        Remove-Item -Path $pathLinkHorizon -Force
    }
}
